...
React's Context API is powerful. Immer makes immutability a breeze. Together? They make state management feel like magic.

Imagine a world where you no longer have to spread your state objects manually or ensure you're not mutating the state directly. With Immer, you just describe the changes you want, and it takes care of the rest.

## Why is this exciting?

Deeply nested states can be a pain to update. With this setup, you can perform updates on nested state objects with ease, without worrying about mutating the original state or dealing with cumbersome object spreading.
...
import { useSiteContext } from "./path-to-your-context-file"; // Adjust this import path.

function ExampleComponent() {
  const { state, setState, dispatch } = useSiteContext();

  // ... (code for state update functions)
  ...
  return (
    <div>
      ...
      {/* ... (rest of the buttons and their corresponding code previews) */}
    </div>
  );
}
